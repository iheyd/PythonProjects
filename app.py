...
st.markdown("""
Моделирование по методу турбулентных струй (согласно конспекту лекций, стр. 1–12).  
...
B = st.sidebar.number_input("Полная ширина русла после расширения B, м", value=10.0, min_value=b*2)
...
x_sections = [float(x.strip()) for x in x_input.split(",") if x.strip()]

Q0 = 2 * b * V0  # Начальный расход на единицу глубины (стр. 7)

# 🔢 РАСЧЁТНОЕ ЯДРО
def kramp(z):
    """Функция Крампа: Φ(z) = 0.5 * (1 + erf(z/√2)) (стр. 9)"""
    return 0.5 * (1.0 + erf(z / np.sqrt(2.0)))

def V2_unbounded(y, x, b, C, V0):
    """Базовое уравнение для безграничного расширения (стр. 5, ур. 5)"""
    Z1 = (y - b) / (2 * C * x)
    Z2 = (y + b) / (2 * C * x)
    return V0**2 * (kramp(Z2) - kramp(Z1))

def solve_xi_for_x(x, y_mid, b, B, C, V0, Q0, tol):
    """Итерационный подбор ξ до совпадения расхода (стр. 11)"""
    def residual(xi):
        V2 = np.zeros_like(y_mid)
        for i, y in enumerate(y_mid):
            # Метод наложения для ограниченного пространства (стр. 10)
            V2[i] = V2_unbounded(y, x, b, C, V0) + V2_unbounded(B - y, x, b, C, V0) - xi
        # Расчёт с учётом знака (стр. 11: обратное течение даёт отрицательный расход)
...
if st.button("🔍 Рассчитать эпюры", type="primary"):
    if not x_sections:
        ...
        with tab2:
            ...
            st.download_button("⬇️ Скачать таблицу (.csv)", data=csv, file_name=f"raschet_x{x_ref}.csv", mime="text/csv")